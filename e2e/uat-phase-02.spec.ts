...
test.describe('Phase 02 UAT', () => {
  ...
  test('FadeUp: CV and Contact sections animate in on scroll', async ({ page }) => {
    ...
    expect(parseFloat(finalOpacity), 'CV section should be visible after scroll').toBeGreaterThan(
      0.9,
    )
  })

  // UAT-3: Reduced motion — animations skipped
  test('reduced motion: CV and Contact appear immediately without animation', async ({
    browser,
  }) => {
    ...
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, '03-reduced-motion.png'),
      fullPage: true,
    })
    ...
    expect(
      parseFloat(cvOpacity),
      'CV should be fully visible immediately under reduced motion',
    ).toBeGreaterThan(0.9)
  ...
  test('header transitions from transparent to surface color after 100px scroll', async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 800 })
    await page.goto('/')
