...
test.describe('Phase 03 UAT', () => {
  ...
  test('projects section renders with eyebrow, h2, and at least one card', async ({ page }) => {
    ...
    await expect(page.locator('section#projects h2#projects-heading')).toHaveText(
  ...
  test('archived repos do not appear in rendered cards', async ({ page }) => {
    ...
    for (const archivedName of ARCHIVED_REPO_NAMES) {
      expect(
        renderedNames,
        `archived repo ${archivedName} must not appear in rendered cards`,
      ).not.toContain(archivedName)
    }
  ...
  test('live demo link presence per card matches source-of-truth homepage value', async ({
    page,
  }) => {
    await page.goto('/')
    await page.waitForLoadState('networkidle')

    const homepageByRepoUrl = new Map<string, string | null>(
      FALLBACK.map((p) => [p.repoUrl, p.homepage]),
    )
    ...
    for (const card of cards) {
      ...
      expect(
        await repoLink.count(),
        'each card must contain exactly one Repo link by accessible name',
      ).toBe(1)
      const repoUrl = await repoLink.getAttribute('href')
      expect(repoUrl, 'every card must have a Repo link href').not.toBeNull()
      expect(repoUrl!, 'Repo link must point at github.com/axelw/...').toMatch(
        /^https:\/\/github\.com\/axelw\//,
      )
      ...
      if (homepageByRepoUrl.has(repoUrl!)) {
        ...
        if (expectedHasLiveDemo) {
          ...
          expect(demoHref, 'Live demo href must equal source-of-truth homepage').toBe(
            expectedHomepage,
          )
        }
      }
  ...
  test('header nav: clicking #projects scrolls projects section into view under the sticky header', async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 800 })
    await page.goto('/')
  ...
  test('reduced motion: projects FadeUp wrapper is visible immediately under prefers-reduced-motion', async ({
    browser,
  }) => {
